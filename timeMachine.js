...
function start() {
    ...
    document.getElementById("website-link").addEventListener("click", function() {
        shell.openExternal("https://crumblingstatue.github.io/FloweysTimeMachine/");
    });
    initializeBackups();
...
function initializeBackups() {
    var UNDERTALEdataDir;
    if (process.platform == "win32") { UNDERTALEdataDir = homdir + "/AppData/Local/UNDERTALE"; }
    else { UNDERTALEdataDir = homdir + "/.config/UNDERTALE_linux_steamver"; }

    if (!checkIfBackupsAreInitialized) { startupBackups(); }

    function checkIfBackupsAreInitialized() {
        if (existsSync(UNDERTALEdataDir + "/backups")) {
            return true;
        } else {
            return false;
        }
    ...
    function startupBackups() {
        mkdirSync(UNDERTALEdataDir + "/backups");
        writeFileSync(UNDERTALEdataDir + "/savename", "Default Backup", { encoding: "utf-8" });
        initializeBackups(); // Call again to doover process
    }
}
